...
class DistributedBatchNorm2d():
    ...
    def __call__(self, num_features):
        return BatchNorm2d(num_features=num_features, world_size=self.world_size)
...
class _BatchNorm(_NormBase):
    ...
    def __init__(self, num_features, world_size=8, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        if not (type(world_size) is int and world_size > 0):
            raise ValueError("world_size must be a positive integer")
        else:
            print("virtual_world_size:"+str(world_size))
